...
namespace blog.Services
{
    ...
    {
        public async Task<PageResponseDto<SequencerListRequestDto>> Get8BitListAsync(PageDto queryDto, CancellationToken ct = default)
        {
            var entity = await repository.GetSequencerNoInclude().Page(queryDto.PageIndex, queryDto.PageSize).ToListAsync(ct);
        ...
        public async Task<SequencerResponseDto> Get8BitDetailAsync(int id, CancellationToken ct = default)
        {
            return await repository.GetSequencerNoInclude().ProjectTo<SequencerResponseDto>(mapper.ConfigurationProvider)
