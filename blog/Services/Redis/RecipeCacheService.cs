...
{
    public class RecipeCacheService(IDistributedCache cache, RecipeRepository recipeRepository, CacheHelper cacheHelper)
    {
        public async Task<RecipeDetailResponse?> GetRecipeDetailAsync(int id, CancellationToken ct = default)
        {
            ...
            return await cacheHelper.SaveCacheAsync<RecipeDetailResponse>(key, () => recipeRepository.GetRecipesNoInclaude(), x => x.Id == id, ct);            
        }
